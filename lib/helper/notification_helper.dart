...
class NotificationHelper {
  ...
  static Future<void> initialize(FlutterLocalNotificationsPlugin flutterLocalNotificationsPlugin) async {
    ...
    flutterLocalNotificationsPlugin.initialize(settings: initializationsSettings, onDidReceiveNotificationResponse: (load) async{
      try{
        if(load.payload!.isNotEmpty){
          ...
          if(payload.notificationType == NotificationType.order || payload.notificationType == NotificationType.assign || payload.type == NotificationType.assign.name){
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) {
      ...
      }else if(message.data['type'] == 'delivery_nam_shift_update'){
        Get.find<AuthController>().updateAndTopicSubscrive();
        NotificationHelper.showNotification(message, flutterLocalNotificationsPlugin);
      }else {
