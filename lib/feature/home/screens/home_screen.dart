...
import 'package:stackfood_multivendor_driver/feature/dashboard/widgets/cash_in_hand_bottomsheet.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _payableBottomSheet() async {
    bool isFreelencer = Get.find<ProfileController>().profileModel != null && Get.find<ProfileController>().profileModel!.earnings == 1;
    bool validPaymentBalance = Get.find<ProfileController>().profileModel != null && Get.find<SplashController>().configModel!.minAmountToPayDm! < Get.find<ProfileController>().profileModel!.payableBalance!;

    if(isFreelencer && validPaymentBalance && !Get.find<SplashController>().hasShownCashInHandBottomSheet) {
      Get.find<SplashController>().setCashInHandShown(true);
      showCustomBottomSheet(child: CashInHandBottomSheet());
    }
  }
  ...
  Future<void> _loadData() async {
    ...
    await Get.find<ProfileController>().getProfile().then((_) {
      _payableBottomSheet();
    });
            ...
            Expanded(
              child: SingleChildScrollView(
                ...
                child: GetBuilder<ProfileController>(builder: (profileController) {
                  ...
                  // if(profileController.profileModel != null && showCashInHandSheet){
                  // print('=========here coming-----2-- ${profileController.profileModel}');
                  // _payableBottomSheet();
                  // }
