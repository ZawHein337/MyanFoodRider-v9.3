...
import 'package:stackfood_multivendor_driver/common/widgets/custom_loader_widget.dart';
import 'package:stackfood_multivendor_driver/feature/notification/controllers/notification_controller.dart';
import 'package:stackfood_multivendor_driver/feature/notification/widgets/not_found_order_bottom_sheet.dart';
                    ...
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      InkWell(
                        onTap: () {
                          ...
                          if (notificationController.notificationList![index].data?.type == 'order_status') {
                            Get.dialog(const CustomLoaderWidget());
                            orderController.getOrderWithId(notificationController.notificationList![index].data?.orderId!).then((value) {
                              Get.back();

                              if (orderController.orderModel != null) {
                                Get.toNamed(
                                  RouteHelper.getOrderDetailsRoute(orderController.orderModel!.id!),
                                  arguments: OrderDetailsScreen(orderId: orderController.orderModel!.id!, isRunningOrder: true, orderIndex: 0),
                                );
                              }else{
                                showCustomBottomSheet(child: NotFoundOrderBottomSheet());
                              }
                            });
                          }else {
                            showDialog(
                            ...
                            Expanded(
                              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                ...
                                Row(children: [
                                  ...
                                  Text(
                                    DateConverter.convertTimeDifferenceInMinutes(notificationController.notificationList![index].updatedAt!),
