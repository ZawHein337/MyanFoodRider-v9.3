...
import 'package:dropdown_button2/dropdown_button2.dart' as db;
...
class _DeliveryManRegistrationScreenState extends State<DeliveryManRegistrationScreen> {
                        ...
                        child: Column(children: [
                          Container(
                            ...
                            child: CustomDropdown<int>(
                              ...
                              child: Padding(
                                ...
                                child: authController.dmTypeIndex == -1 ? Row(
                                  children: [
                                    Text(
                          ...
                          authController.vehicleIds != null ? Container(
                            ...
                            child: CustomDropdown<int>(
                              ...
                              child: Padding(
                                padding: const EdgeInsets.only(left: Dimensions.paddingSizeSmall),
                                child: authController.vehicleIndex == 0 || authController.vehicleIndex! >= authController.vehicles!.length ? Row(
                                  children: [
                                    Text(
                                      'select_vehicle'.tr,
                                      style: robotoRegular.copyWith(color: Theme.of(context).hintColor),
                                  ...
                                  authController.vehicles![authController.vehicleIndex!].type!.tr,
                                  style: robotoRegular.copyWith(color: Theme.of(context).textTheme.bodyLarge!.color),
                                ),
                              ),
                            ),
                          ...
                          (authController.dmTypeIndex == 0) ?
                          (authController.shifts != null && authController.shifts!.isNotEmpty) ?
                          Column(
                              ...
                              children: [
                                ...
                                Stack(clipBehavior: Clip.none, children: [
                                  CustomDropdownButton(
                                    ...
                                    borderWidth: 0.1,
                                    dropdownMenuItems: authController.shifts!.map((shift) {
                                      bool isSelected = authController.selectedShifts.any((deliveryManShift) => deliveryManShift.id == shift.id);
                                      bool isFullDay = shift.isFullDay == 1;
                                      bool hasFullDay = authController.selectedShifts.any((deliveryManShift) => deliveryManShift.isFullDay == 1);
                                      bool hasOtherShifts = authController.selectedShifts.any((deliveryManShift) => deliveryManShift.isFullDay != 1);
                                      bool shouldDisable = isSelected || (isFullDay && hasOtherShifts) || (!isFullDay && hasFullDay);

                                      return db.DropdownItem<ShiftModel>(
                                        value: shift,
                                    ...
                                    selectedItemBuilder: (BuildContext context) {
                                      return (authController.shifts ?? []).map((shift) {
                                        return RichText(
                                          text: TextSpan(children: [
                                            TextSpan(
                                        ...
                                        return Text('working_shift'.tr, style: robotoRegular.copyWith(color: Theme.of(context).hintColor, fontSize: Dimensions.fontSizeDefault));
                                      }).toList();
                                    },
                                    onChanged: (value) {
                                      if (value != null && !authController.selectedShifts.any((s) => s.id == value.id)) {
                                        authController.toggleShift(value);
                                      }
                                    },
                                  ),
                                ...
                                authController.selectedShifts.isNotEmpty ? Column(children: [
                                  const SizedBox(height: Dimensions.paddingSizeSmall),
                                  Wrap(spacing: 8, runSpacing: 8, alignment: WrapAlignment.start, children: List.generate(
                                      authController.selectedShifts.length,
                                      growable: true, (index) {
                                    final shift = authController.selectedShifts[index];
                                    return Chip(
                                      ...
                                      onDeleted: () {
                                        authController.removeShift(shift);
                                      },
  ...
  void _addDeliveryMan(AuthController authController, AddressController addressController) async {
    ...
    if(authController.dmStatus == 0.4) {
      ...
      if(authController.dmTypeIndex == -1) {
      ...
      }else if(authController.dmTypeIndex != 1 && authController.selectedShifts.isEmpty) {
        ...
        data.addAll(DeliveryManBodyModel(
          ...
          vehicleId: authController.vehicles![authController.vehicleIndex! - 1].id.toString(), shiftIds: authController.getSelectedShiftIds(),
        ).toJson());
